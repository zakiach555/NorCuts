...
def call_g4f(prompt, model_name="gpt-4o-mini"):
    ...
    for attempt in range(max_retries):
        try:
            ...
            print(f"[WARN] G4F API error (Attempt {attempt+1}/{max_retries}): {e}")
...
def process_segments(raw_segments, transcript_segments, min_duration, max_duration, output_count=None):
    ...
    for seg in all_segments:
        try:
            ...
            if duration < tempo_minimo: 
                print(f"[WARN] Segment below min duration ({duration:.2f}s < {tempo_minimo}s). Extending to {tempo_minimo}s.")
                duration = tempo_minimo
                final_end_time = final_start_time + duration
            
            # Validate Duration (Max)
            if duration > tempo_maximo:
                print(f"[WARN] Segment exceeds max duration ({duration:.2f}s > {tempo_maximo}s). Trimming to {tempo_maximo}s.")
    ...
    # Basic validation: ensure start_time is present
    validated_segments = []
...
{json_template}"""
    ...
    for i, prompt in enumerate(output_texts):
        ...
        if ai_mode == "manual":
            ...
            print("Or type 'file' to read from 'tmp/response.json'.")
