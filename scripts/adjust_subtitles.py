...
def generate_ass_from_file(input_path, output_path, project_folder,
                           ...
                           font, outline_color, shadow_color, bold, italic, underline,
                            ...
                            speaker_name="", speaker_title="", speaker_duration=4.0):
    """
    ...
    if speaker_name.strip():
        # BorderStyle=1: outline+shadow (no opaque box), heavy outline for readability on any bg
        speaker_box_style = f"\nStyle: SpeakerBox,{font},20,&H0000D7FF&,&H00000000&,&H00000000&,&H00000000&,-1,0,0,0,100,100,0,0,1,2,1,7,15,10,58,1"
...
Style: Default,{font},{base_size},{base_color},&H00000000,{outline_color},{shadow_color},{bold},{italic},{underline},{strikeout},100,100,0,0,{border_style},{outline_thickness},{shadow_size},{current_alignment},10,10,{current_vertical_position},1{speaker_box_style}
...
"""
    ...
    with open(output_path, "w", encoding="utf-8") as f:
        ...
        # Speaker info overlay (optional)
        if speaker_name.strip():
            end_ts = format_time_ass(float(speaker_duration))
            name_safe = speaker_name.strip().replace("\\N", " ").replace("\n", " ")
            title_safe = speaker_title.strip().replace("\\N", " ").replace("\n", " ")
            # \an7 = top-left anchor | \pos(15,58) = ~9% from top, matches reference reel layout
            # Name: large bold gold | Title: smaller white below via \N
            if title_safe:
                speaker_text = f"{{\\an7\\pos(15,58)\\b1\\fs20\\c&H0000D7FF&}}{name_safe}{{\\N\\b0\\fs11\\c&H00FFFFFF&}}{title_safe}"
            else:
                speaker_text = f"{{\\an7\\pos(15,58)\\b1\\fs20\\c&H0000D7FF&}}{name_safe}"
            f.write(f"Dialogue: 1,0:00:00.00,{end_ts},SpeakerBox,,0,0,0,,{speaker_text}\n")
...
def adjust(base_color, base_size, highlight_size, highlight_color, words_per_block, gap_limit, mode, vertical_position, alignment, font, outline_color, shadow_color, bold, italic, underline, strikeout, border_style, outline_thickness, shadow_size, uppercase=False, project_folder="tmp", **kwargs):
    ...
    speaker_duration = kwargs.get('speaker_duration', 4.0)
