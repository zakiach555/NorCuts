...
i18n = I18nAuto()
# Subtitle colors in BGR format (Blue-Green-Red) for ASS
COLORS = {
    "red": "0000FF",
    "yellow": "00FFFF",
    "green": "00FF00",
...
def get_subtitle_config(config_path=None):
    ...
    config = {
        ...
        "gap_limit": 0.3,
        "mode": "highlight",
        "highlight_color": f"&H{highlight_color_transparency}{COLORS['yellow']}&",
...
def main():
    parser = argparse.ArgumentParser(description="NorCuts CLI")
    ...
    parser.add_argument("--no-face-mode", choices=["padding", "zoom"], default="padding", help="Method to handle segments with no face detected")
    parser.add_argument("--face-detect-interval", type=str, default="0.17,1.0", help="Face detection interval in seconds")
    ...
    parser.add_argument("--face-two-threshold", type=float, default=0.60, help="Relative area threshold to trigger 2-face mode")
    parser.add_argument("--face-confidence-threshold", type=float, default=0.30, help="Face detection confidence threshold")
    parser.add_argument("--face-dead-zone", type=str, default="40", help="Camera movement dead zone in pixels")
    ...
    parser.add_argument("--active-speaker-mar", type=float, default=0.03, help="Mouth Aspect Ratio threshold for active speaker")
    parser.add_argument("--active-speaker-score-diff", type=float, default=1.5, help="Score difference to focus on active speaker")
    parser.add_argument("--include-motion", action="store_true", help="Include motion in activity score")
    parser.add_argument("--active-speaker-motion-threshold", type=float, default=3.0, help="Motion deadzone in pixels")
    parser.add_argument("--active-speaker-motion-sensitivity", type=float, default=0.05, help="Motion sensitivity multiplier")
    parser.add_argument("--active-speaker-decay", type=float, default=2.0, help="Activity score decay rate")
    ...
    parser.add_argument("--translate-target", help="Target language code for subtitle translation (e.g. 'ar', 'en').")
    ...
    # Early check for existing viral segments (to skip config prompts)
    viral_segments = None
    ...
    # AI config vars (only needed if we don't have existing segments)
    num_segments = None
    viral_mode = False
    themes = ""
    ai_backend = "manual"
    ...
    if not viral_segments:
        ...
        if ai_backend == "gemini" and not api_key:
            ...
            if cfg_key and cfg_key != "YOUR_KEY_HERE":
                api_key = cfg_key
