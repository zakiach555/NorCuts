...
def run_viral_cutter(input_source, project_name, url, video_file, segments, viral, themes, min_duration, max_duration, model, ai_backend, api_key, ai_model_name, chunk_size, workflow, face_model, face_mode, face_detect_interval, no_face_mode,
                     ...
                     enable_speaker_box=False, speaker_name="", speaker_title="", speaker_duration=4.0):
    ...
    if use_custom_subs:
        ...
        if enable_speaker_box and speaker_name.strip():
            subtitle_config["speaker_name"] = speaker_name.strip()
            subtitle_config["speaker_title"] = speaker_title.strip()
            subtitle_config["speaker_duration"] = float(speaker_duration)
        else:
            subtitle_config["speaker_name"] = ""
            subtitle_config["speaker_title"] = ""
            subtitle_config["speaker_duration"] = 4.0
...
css = """
/* ===== NorCuts — Professional Dark Theme ===== */
...
/* ── Checkboxes & Sliders ── */
input[type="checkbox"] { accent-color: #f59e0b !important; }
...
/* ── HTML Results ── */
.gr-html { border-radius: 10px !important; overflow: hidden !important; }

/* ── Markdown headers ── */
.prose h3 { color: #f59e0b !important; font-size: 1em !important; font-weight: 700 !important; margin: 8px 0 4px !important; }

/* ── Info text ── */
.info-text, .description { color: #64748b !important; font-size: 0.78em !important; }
...
/* ── Number inputs arrow styling ── */
input[type="number"]::-webkit-inner-spin-button { opacity: 0.5; }

/* ── Group / Row containers ── */
.gap, .form { gap: 8px !important; }

/* Gradio block borders */
.block { border-color: #1e1e35 !important; background: transparent !important; }
"""
...
with gr.Blocks(title=i18n("NorCuts WebUI"), theme=gr.themes.Default(primary_hue="orange", neutral_hue="slate"), css=css) as demo:
    ...
    with gr.Tabs():
        with gr.Tab(i18n("Create New")):
             with gr.Row():
                with gr.Column(scale=1):
                    ...
                    with gr.Row():
                        ...
                        translate_input = gr.Dropdown(choices=["None", "ar", "en", "pt", "es", "fr", "de", "it", "ru", "ja", "ko", "zh-CN"], label=i18n("Translate Subtitles To"), value="ar")
             ...
             with gr.Accordion(i18n("Subtitle Settings (alpha)"), open=False):
                preset_input = gr.Dropdown(choices=[(i18n("Manual"), "Manual")] + [(i18n(k), k) for k in subs.SUBTITLE_PRESETS.keys()], label=i18n("Quick Presets"), value="Norz Arabic (Motivational)")
                ...
                with gr.Accordion(i18n("Advanced Settings"), open=False):
                    ...
                    with gr.Row():
                        font_name_input = gr.Textbox(label=i18n("Font Name"), value="Cairo")
                        font_size_input = gr.Slider(label=i18n("Font Size (Base)"), minimum=8, maximum=80, value=36)
                        highlight_size_input = gr.Slider(label=i18n("Highlight Size"), minimum=8, maximum=80, value=44)
                ...
                with gr.Accordion(i18n("Speaker Info Overlay (Optional)"), open=False):
                    gr.Markdown(f"<span style='color:#64748b; font-size:0.85em'>{i18n('Show a speaker intro box in the top-left corner at the start of each clip.')}</span>")
                    enable_speaker_box = gr.Checkbox(label=i18n("Enable Speaker Box"), value=False)
                    with gr.Row():
                        speaker_name_input = gr.Textbox(label=i18n("Speaker Name"), placeholder=i18n("e.g. David Goggins"), value="", interactive=True)
                        speaker_title_input = gr.Textbox(label=i18n("Speaker Title / Role"), placeholder=i18n("e.g. Navy SEAL · Author"), value="", interactive=True)
                    speaker_duration_input = gr.Slider(label=i18n("Display Duration (seconds)"), minimum=1.0, maximum=12.0, value=4.0, step=0.5)
             ...
             start_btn.click(run_viral_cutter, inputs=[
                 ...
                 enable_speaker_box, speaker_name_input, speaker_title_input, speaker_duration_input
    ...
    gr.Markdown(f"""
        <div style='text-align: center; padding: 16px 0 8px; border-top: 1px solid #1e1e35; margin-top: 12px;'>
            <span style='color: #334155; font-size: 0.82em; letter-spacing: 1px;'>
                ✂️ <strong style='color: #f59e0b;'>NorCuts</strong> &nbsp;·&nbsp; by <strong style='color: #94a3b8;'>Norz</strong> &nbsp;·&nbsp; {i18n('100% local • open source • no subscription required')}
            </span>
        </div>
        """)
